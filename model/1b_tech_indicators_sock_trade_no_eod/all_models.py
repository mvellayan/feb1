"""
all_models.py — 1b_tech_indicators_sock_trade_no_eod

Orchestrates all 1,221 indicator combinations across 100 time windows.
Identical to 1a except positions are never forced out at end of day —
the only exit is the ATR trailing stop.  If the stop does not fire before
the window boundary the trade closes with exit_reason='window_end'.
...
sys.path.insert(0, str(_MODEL1A))  # utils from model 1a
sys.path.insert(0, str(_HERE))     # single_model from this directory
...
def run_model_set(
    ...
    print(f"\n{'='*60}")
    print(f"[{seq_no}]  AAPL Intraday Backtest  [no-EOD exit]  |  start at {ts}")
    print(f"[{seq_no}]  Window: {date_start} -> {date_end}  seed: {seed}  exit: trailing-stop-only")
    print(f"{'='*60}\n")
    ...
    with open(log_path, 'w', encoding='utf-8') as log_fh:
        log_fh.write(
            ...
            f"  seed: {seed}  exit_mode: [trailing-stop-only]  generated: {ts}\n"
        )
...
def write_markdown_report(models_df, category_dfs, pair_dfs, n_batches, total_rows, run_dir):
    ...
    lines = []
    lines.append('# Batch Run Analysis — Indicator Consistency Report  [no-EOD exit]')
    lines.append(f'\n_Generated: {now}_\n')
    ...
    lines.append(f'| Exit mode               | trailing-stop only (no EOD forced exit) |')
    lines.append('')
...
if __name__ == '__main__':
    _parser = argparse.ArgumentParser(
        description='Run all 1,221 models across 100 windows (no EOD forced exit).'
    )
    ...
    print(f"[main]  Exit mode   : trailing-stop only (no EOD forced exit)")
