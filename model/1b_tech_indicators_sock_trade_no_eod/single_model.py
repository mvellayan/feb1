"""
single_model.py — 1b_tech_indicators_sock_trade_no_eod

Core trading simulation engine. Identical to 1a except for exit logic:
positions are NEVER forced out at the end of a trading day.  The only exit
is the ATR-based trailing stop.  If the stop does not fire before the window
boundary the position is closed with exit_reason='window_end'.

Engine functions (imported by all_models.py)
───────────────────────────────────────────────────────
  load_or_build_signals()
  draw_sample(df, seed)
  compute_bracket(entry_avg, atr)
  compute_shares(entry_price)
  simulate_trade(df_slice, entry_iloc, stop, target, sell_cols,
...
──────
  ...
  single_{combo}_trades.csv  – every trade across all runs
...
sys.path.insert(0, str(_MODEL1A))  # signals/utils from model 1a
sys.path.insert(0, str(_HERE))     # local modules take precedence

from signals import add_buy_signals, add_sell_signals
from utils import REPORTS_DIR as _REPORTS_DIR_1A, PF_CAP, md_table
...
def simulate_trade(
    ...
    """
    Walks forward from entry_iloc+1 through the end of df_slice (multi-day).

    df_slice spans from the entry bar's trade date to the end of the window,
    so the position carries across day boundaries until the trailing stop fires.
    ...
    Exit:
      stop_loss   — trailing stop triggered (bar's average <= trailing_stop)
      window_end  — stop never fired; position closed at end of window
    """
    exit_price  = None
    exit_reason = None
    exit_bar    = None
    ...
    for i in range(entry_iloc + 1, len(df_slice)):
        ...
        # Trailing stop-loss — triggered when WAP falls to/below stop
...
def _build_from_entry_context(
    df: pd.DataFrame,
    entry_global_idx: int,
) -> tuple[pd.DataFrame | None, int | None]:
    """
    Build a DataFrame slice spanning from the entry bar's trading day through
    the end of df (the window boundary).  The trade carries day-by-day until
    the trailing stop fires or the window expires.
...
def run_combo(
    ...
    Each trade spans from its entry bar to the trailing stop trigger or the
    end of the window — it is never cut short at end of day.
    ...
    for idx in fired_idx:
        ...
        # Build a slice from entry day through end of window (no EOD cut-off)
        df_sim, entry_iloc_sim = _build_from_entry_context(df, idx)
        ...
        trades.append({
            ...
            **{cat: indicators.get(cat, '') for cat in ['trend', 'momentum', 'volatility', 'volume']},
            'trade_date':    str(row['fnd_trade_date']),
...
def parse_args() -> tuple[dict[str, str], int, datetime.date, datetime.date, int]:
    parser = argparse.ArgumentParser(
        description='Run a single indicator combination across N time windows (no EOD exit).'
    )
...
def generate_test_runs(n: int = N_RUNS, meta_seed: int = META_SEED) -> list[tuple[str, str, int]]:
    ...
    if total_days < n:
        raise ValueError(
            f"Date range {DATA_FIRST} → {DATA_LAST} only spans {total_days} usable start days, "
            f"but {n} non-overlapping {WINDOW_DAYS}-day windows were requested.  "
            f"Widen the date range or reduce N_RUNS."
        )
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  Single Model Batch Test  [no-EOD exit]")
    print(f"{'='*60}")
    print(f"  Indicators : {indicators}")
    print(f"  Active cats: {list(active.keys())}")
    print(f"  Exit mode  : trailing-stop only (no EOD)")
    ...
    with open(log_path, 'w', encoding='utf-8') as log_fh:
        log_fh.write(
            ...
            f"  exit_mode: [trailing-stop-only]"
