"""
all_models.py — 4_covered_calls

Random-entry covered-call strategy.  No indicator signals.

Entry
─────
For each window: draw random bars from the session-valid set.  For each active
(expiry_label, strike_label) variant, look up the sold-call bid at entry_ts and
compute the opening time value:
...
Sample counter (shared across variants):
  - A draw counts toward --sample_size iff at least one variant has a valid quote.
  - no-quote draws (all variants stale) do NOT consume the budget.
  - tv-range fails, cooldown skips, and accepted entries DO consume the budget.

Batch counter (per variant):
  - --batch_size is the per-variant cap on accepted entries.
  - Window ends when sample_size is reached OR every variant has filled its batch.
...
_extended_cache: pd.DataFrame | None = None
...
def load_extended() -> pd.DataFrame:
    ...
    print(f"[data] Loading {EXTENDED_CSV} ...")
    df = pd.read_csv(
        EXTENDED_CSV, parse_dates=['date'],
...
def prepare_window(
    ...
    """Filter to window, apply session/spike filters, shuffle valid indices."""
    mask = (
    ...
    if len(valid_idx) == 0:
        return None, 'no_valid_bars'

    rng = np.random.default_rng(seed)
    shuffled = rng.permutation(valid_idx).tolist()
    return (df, shuffled), 'ok'
...
def run_window_batch(
    ...
    """
    Execute one window.  Returns (summary_df, trades_df, sample_count).
    """
    var_state = {
        key: {
            'last_entry_ts':  None,
    ...
    for global_idx in shuffled_idx:
    ...
    return summary_df, trades_df, sample_count
...
def run_model_set(
    ...
    if window_data is None:
        ...
        return
    df, shuffled_idx = window_data
    print(f"[{seq_no}][filter]  {date_start} -> {date_end}: {len(df):,} rows, "
          f"{len(shuffled_idx):,} valid bars")
    ...
    with open(log_path, 'w', encoding='utf-8') as log_fh:
        log_fh.write(
            ...
            f"variants: {len(variants_selected)}  "
            f"sample_size: {sample_size}  batch_size: {batch_size}  "
            f"cc_tv: [{cc_tv_min}, {cc_tv_max}]  buyback_tv: {buyback_tv}\n"
        )
        summary_df, trades_df, sample_count = run_window_batch(
            df, shuffled_idx, variants_selected,
            sample_size, batch_size, cc_tv_min, cc_tv_max, buyback_tv,
            seq_no, log_fh=log_fh,
        )
    print(f"[{seq_no}][report]  Log      -> {log_path}")
    print(f"[{seq_no}]          sample_count used: {sample_count:,}/{sample_size:,}")
...
def write_markdown_report(
    ...
    lines.append(f'| Sample size / win   | {sample_size:,} |' if sample_size else '| Sample size / win   | — |')
...
if __name__ == '__main__':
    _parser = argparse.ArgumentParser(
        description='Run random-entry covered-call strategy across N windows '
                    '(matrix of expiry × strike variants, no indicator signals).'
    )
    ...
    _parser.add_argument('--sample_size', type=int, default=10_000,
                         help='Number of random quotes to test per window (default: 10000)')
    ...
    print(f"[main]  Sample size : {_args.sample_size:,} quotes/window")
    ...
    print("[main]  Loading extended CSV and option index ...")
    load_extended()
    load_option_index()
    ...
    for seq_no, (offset, seed) in enumerate(runs, 1):
        ...
        window_args.append((
            ...
            _args.sample_size, _args.batch_size,
